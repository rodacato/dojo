...
// ---------------------------------------------------------------------------
// Scroll: TypeScript Fundamentals
// ---------------------------------------------------------------------------
...
// before the greet kata that uses them. UUID is brand new — no
// historical progress to preserve.
const STEP_1_2_ID = seedUuid('ts-step-1-2-template-literals')
// Existing UUIDs preserved so anonymous/user progress on these katas
...
// Sprint 018: every sub-scroll needs at least one challenge per §4.3 of
// docs/scrolls/README.md. Palindrome is bumped to challenge and a second
...
const LESSONS_DATA = [
  { id: LESSON_1_ID, scrollId: COURSE_ID, order: 1, title: 'Variables & Types' },
  { id: LESSON_2_ID, scrollId: COURSE_ID, order: 2, title: 'Arrays & Objects' },
  { id: LESSON_3_ID, scrollId: COURSE_ID, order: 3, title: 'Control Flow' },
]
...
//   - new:    `__DOJO_RESULT__ <json>` line the ScrollPlayerPage parser trusts
...
For this kata, treat \`fn\` as a unary function whose argument is JSON-serialisable (use \`JSON.stringify\` to build the cache key).`,
