...
- **Ask the sensei MVP** (single-shot nudge, PRD 026) — `LLMPort.generateNudge` + `GenerateNudge` use case + `POST /learn/nudge` + rate limiter + `COURSE_NUDGE_ENABLED` flag + inline UI. Persistence in the same sprint: `step_nudges` table + 👍👎 feedback. No memory, no threaded chat — full version deferred to S021+ if usage signals it.
