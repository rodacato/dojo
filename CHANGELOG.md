...
**Piston survives a reboot whose old cgroup is still busy.** Every deploy reboots the Piston accessory, and since the 2026-09-14 00:24 UTC deploy it had been restart-looping on `mkdir: cannot create directory 'isolate/': File exists`: Liveness and the execute smoke went red and code execution was down. The `rmdir` meant to clear the leftover `isolate/` cgroup fails silently when the old container's cgroup is still busy. The accessory no longer calls the image entrypoint. It runs the same cgroup setup with `mkdir -p`, so a leftover subtree gets reused instead of crashing the container.
...
**Email no longer sends as my domain by default.** `RESEND_FROM_EMAIL` defaulted to `dojo <noreply@notdefined.dev>` — the reference instance's domain. Any self-hoster who set `RESEND_API_KEY` and forgot the sender was silently sending welcome emails and access requests from a domain they don't own, which Resend rejects unless it's verified in *their* account. The variable has no default now and is required whenever `RESEND_API_KEY` is set: the API refuses to boot and names it. Deriving it from `APP_HOST` was considered and dropped — the web hostname (`dojo.example.com`) is rarely the verified sending domain (`example.com`). Email stays fully opt-in: with the key empty nothing is required, the API boots, and `POST /access-requests` still answers 503. The deploy workflow enforces the same rule before Kamal runs — otherwise the only symptom of a `production` environment that has the key but not the sender would be a healthcheck timeout minutes into `kamal deploy`, with the real reason buried in the container logs.
...
**Design-system contrast + print-safe ink strokes.** Added a `--color-on-accent` token (ink-white per theme) for any foreground sitting on a filled `bg-accent` — button labels, the user chat bubble, the active segment, hanko caps, plus the hand-rolled accent buttons on login, invite redeem, engawa, the scroll-complete banner, error states, and the feedback box. Before, those reused `--color-primary`, which inverts to near-black under the washi theme and dropped the text to ~3:1 on vermillion (fails AA). All eleven filled-accent surfaces now read `text-on-accent`, which also makes the hanko caps genuinely ink-white in dark themes as the spec always claimed. Separately, the VerdictBlock's vertical brushstroke now rests *drawn* (`stroke-dashoffset: 0`) and only animates from hidden under `prefers-reduced-motion: no-preference`, so print, SSR and reduced-motion render the stroke visible instead of blank — no more reliance on `animation-fill-mode` to hold the final frame.
