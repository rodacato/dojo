...
- **This jumped the gate S030 reserves.** The planned sequence was *full-set smoke → publish decisions*; we published first, on Adrian's call. So the protective gate still hasn't run and is now urgent, not stretch: the **full-set real-Piston smoke** (Ruby especially — it changed here, no local runtime to smoke against) and the **TS Piston deploy config** (`max_run_timeout`≥8000 + `output_max_size`, else TS katas time out — flagged loudly at `TYPESCRIPT_COURSE_DATA`). The seed flip ships them ready on reseed; it does **not** bypass the smoke. Both carry into S030.
