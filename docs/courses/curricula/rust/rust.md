...
- **Test harness:** **manual `_t`/`_eq` pattern, consistent with Ruby and Python** (S028 decision — the draft's manual-vs-`cargo test` open question is closed). Shape sketch, finalized at seed:
  ...
  ```
  `_t` is two-argument by design: every lesson's `testCode` is written as bare `_t("sentence", || ...)` calls with no results accumulator in sight, so the harness must own it — a thread-local results vec inside the harness, not a third parameter threaded through every call. `main` runs the `_t` calls and emits the `__DOJO_RESULT__ <json>` footer (JSON assembled with `format!` — no `serde`). `catch_unwind` keeps one panicking test from killing the suite. A float-epsilon helper (`_eq_close`) ships for kata 5.3. Test names are user-facing sentences, per framework rules. **Resolved at the L1 smoke (2026-06-12, real Piston rustc 1.68.2):** the learner-`main`-vs-harness-`main` merge is handled by the **PistonAdapter renaming a learner-written `fn main` to `__learner_main`** — learner code stays at the file's top level (same-file items need no `pub`), zero line-number offset, and the playground's testCode calls `__learner_main()` to run the learner's entry. A `mod solution` wrap was implemented first and failed at the smoke on module privacy (`E0425`/`E0603`) — kept here so it isn't re-proposed.
- **Fail-by-design katas and the compile verdict:** for katas 1.3, 1.4, and 2.5 the starter intentionally fails to compile; ExecuteStep must surface the compile error as the feedback channel (stderr passthrough), not as an infrastructure failure. **Validated at the L1 smoke (2026-06-12):** 1.3 and 1.4 starters produce their full `E0382` (anatomy + `help: consider cloning` — which exists on 1.68.2, settling the draft's open question) through the compile-stderr channel; the adapter scrubs Piston's own noise lines (`Sandbox keeper…`, `chmod: cannot access 'binary'`) and normalizes the null exit code Piston reports when the compile stage dies on a signal. Solutions pass 5/5 against the real harness.
...
- **`✓ resolved` — Compile-error surfacing for fail-by-design katas.** Validated at the L1 smoke (2026-06-12) against real Piston rustc 1.68.2: compile errors surface as learner feedback through ExecuteStep's stderr channel; the adapter scrubs Piston's internal noise and normalizes signal-death exit codes. Entry-point strategy: adapter renames learner `fn main` → `__learner_main` (the `mod solution` wrap failed on module privacy — E0425/E0603). Batches 2-4 are unblocked; each batch recaptures its own error excerpts live before seeding.
...
- **`✓ resolved` — Lesson titles language.** Settled before this question was even written: the Python Spanish titles were a bug fixed at the S027 smoke (seed commit 2026-06-11 translated all four to English); product content is English suite-wide. Rust seeds English from day one.
