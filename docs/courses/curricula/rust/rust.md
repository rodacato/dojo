...
- **voice_check:** ends with the `&str`-default paragraph's compiling sample, not prose; the `E0499` anchor stays headline-line-only per the pairing clause; no lifetimes content in this read.
...
- **Quoted error output:** every quoted `rustc` message is captured from Piston's actual 1.68.2 during the Lesson 1 smoke batch and pasted into the seeds. Full multi-line output: 0.1 (3-line `E0382` teaser), 1.2, 2.2, 2.4 (the fresh-snippet `E0499` walk — a second `E0499` capture, distinct from 2.2's — + the 3-line `E0502` excerpt), 3.1 (`E0277`, `?` form), 4.2 (`E0277`, unsized form), 5.2. Headline line only (pairing clause, §2.2 rule 2): 1.1, 2.1, 5.1. Error codes are stable; `help:`/`note:` phrasing drifts across versions — never trust training-data memory of an error string.
