...
- **Test harness:** **manual `_t`/`_eq` pattern, consistent with Ruby/Python/Rust** (the scratch draft's "use `go test`, no manual harness" is void — `go run`, not `go test`). Shape sketch, finalized at seed:
  ...
  - **ECHO-FREE FOOTER — specified from the start (S029, the bug just removed from the other three footers).** `main` runs the `_t` calls and emits **ONLY** the single `__DOJO_RESULT__` JSON line — **no per-test `✓`/`✗` echo to stdout.** The per-test echo tips Piston's stdout cap (`PISTON_OUTPUT_MAX_SIZE`) and was removed from Ruby/Python/Rust; Go must never add it. Footer:
    ```go
    b, _ := json.Marshal(map[string][]_result{"tests": _results})
    fmt.Println("__DOJO_RESULT__ " + string(b))
    ```
  - **Entry-point merge:** mirror Rust's resolution — the PistonAdapter renames a learner-written `func main` to `__learnerMain` (or the adapter wraps the learner code so the harness owns `main`); confirm the exact mechanism at the L1 smoke. Learner code stays at file top level (same-package items need no export). **Validate the merge before seeding any lesson** — this is the highest-risk infra item (Rust's `mod solution` wrap failed on privacy; Go's package-level visibility is simpler, but smoke it).
...
- **Stdlib only.** Confirmed-needed packages (verify each exists on 1.16.2 at smoke — all should): `errors`, `fmt`, `io`, `bufio`, `bytes`, `strings`, `strconv`, `sort`, `time`, `sync`, `reflect`, `encoding/json`. **NOT available / NOT used:** `slices`, `maps`, `cmp` (1.21), generics (1.18), `errors.Join` (1.20). `net/http`/`httptest` are **not used** (the scratch's httptest lessons are cut — §2.4, §4 Lesson 5 — partly because loopback networking in Piston is unconfirmed and partly because it pulled the scroll past budget).
