...
- **Turnstile on `/scrolls/execute` for anonymous callers** (precautionary, 2026-06-19) — the anon execution path is already protected by per-IP rate limiting + the language whitelist + the Piston sandbox; the playground (`/engawa/run`) additionally gates anon runs behind Cloudflare Turnstile. Mirror that on `/scrolls/execute` if/when execution abuse is actually observed (logs / cost). Decided NOT to gate scrolls behind auth — that reverses the free/anonymous thesis (ADR 022, README §1) and identity doesn't stop automated abuse; rate-limit + captcha + sandbox quotas do (Marta C5). Trigger: real abuse signal. (Marta C5)
- **Scrolls catalog + orientation landing reshape** (S030 active — Phase 0 + catalog + landing in S030; player consistency pass + completion + error states may carry to S031) — presentation-only pass bringing `/scrolls` and the scroll-entry experience to the Rustlings-style clarity Adrian endorsed 2026-06-19 (clear contract, time-on-card, binary progress state + filters on the catalog, a new orientation landing at `/scrolls/:slug` with free lesson navigation). Engine untouched; reuse site tokens + existing components. Excludes streak (parked, README §10). Prereqs: `estimatedMinutes` schema field + batch `/scrolls/progress` endpoint. Design + plan staged in workspace-local notes (Soren C6 + Maya S11 + Priya C1)
